...
class SparsePruner(object):
    ...
    def pruning_mask_twice(self, weights, previous_mask):
        """
        Ranks the remaining weights by magnitude. Set the value at the corresponding position in the mask to
    ...
    def apply_mask_prune(self, masks):
        for module_idx, module in enumerate(self.model.shared.modules()):
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                weight = module.weight.data
                mask = masks[module_idx].cuda()
                weight[mask.eq(0)] = 0.0

    def apply_mask_prune_once(self, masks):
        for module_idx, module in enumerate(self.model.shared.modules()):
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                weight = module.weight.data
                mask = masks[module_idx].cuda()
    ...
    def apply_mask_prune_twice(self, masks):
        for module_idx, module in enumerate(self.model.shared.modules()):
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                weight = module.weight.data
                mask = masks[module_idx].cuda()
                weight[mask.eq(0)] = 0.0
                weight[mask.gt(self.current_dataset_idx)] = 0.0
    ...
    def make_fine_tune_mask(self, mode):
        """Turns previously pruned weights into trainable weights for current task."""
